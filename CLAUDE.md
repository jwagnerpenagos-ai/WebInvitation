...
- `src/main.js` — vanilla JS, no dependencies, ES module imported directly via `<script type="module">`. Organized as small init functions called at the bottom of the file (`bindInvitationData`, `setupIntro`, `setupRsvp`, `setupCountdown`, `setupSmoothAnchors`). Key behaviors:
  ...
  - **RSVP**: `#open-rsvp` opens a native `<dialog>` (`#rsvp-dialog`) with a form (name, attending yes/no, guest count — options generated from `config.rsvp.maxGuests`, optional note). On submit it builds a message client-side and opens it as a `wa.me` deep link (`window.open`, no page navigation); no server or database involved. There is no URL-based guest personalization — that was removed in favor of this in-page form.
...
Edit only `src/config.js`. Do not put event-specific data (names, dates, phone numbers, venue) directly into `index.html` or `main.js` — the binding functions in `main.js` read from `config.js` exclusively, and hardcoding elsewhere will drift out of sync with the RSVP logic.
